...
void JYH_Coloca_Parede(/*JYH_GameState* jogo*/JYH_Camera* cam,JYH_Nivel* lvl, SDL_Point* p){
...
void JYH_Apaga_Parede(/*JYH_GameState* jogo*/JYH_Camera* cam,JYH_Nivel* lvl, SDL_Point* p){
	//JYH_Converter_TelaMundo(p,&jogo->le.r_editor,&jogo->le.r_camera);
    JYH_Converter_TelaMundo(p,cam);
	//unsigned char *mat = jogo->le.lvl.mat;
...
int JYH_ZoomIn(JYH_Nivel* lvl,/*SDL_Rect* cam,Uint32* z*/JYH_Camera* cam){//Aumenta o zoom e returna em Booleano se ainda é permitido
	//(*z)*=2;//Assume que a função só é chamada quando pode dar zoom
    cam->zoom *= 2;
	//Uint32 zt = (*z)*2;
    Uint32 zt = (jogo->zoom)*2;
	//return (2*zt > cam->h || 2*zt > cam->w);
    return (2*zt > cam->r_cam.h || 2*zt > cam->r_cam.w);
}
int JYH_ZoomOut(JYH_Nivel* lvl,/* SDL_Rect* cam,Uint32* z*/JYH_Camera* cam){
	//(*z)/=2;
    cam->zoom /= 2;
	//Uint32 zt = (*z)/2;
			...
			case SDL_MOUSEMOTION:
				...
				if(jogo->le.press && SDL_PointInRect(&p,&jogo->le.r_editor)){
					switch(jogo->le.pincel){
						case PINCEL_PINTANDO:/*JYH_Coloca_Parede(jogo,&p)*/JYH_Coloca_Parede(&jogo->le.cam,&jogo->le.lvl,&p);break;
						case PINCEL_APAGANDO:/*JYH_Apaga_Parede(jogo,&p)*/JYH_Apaga_Parede(&jogo->le.cam,&jogo->le.lvl,&p);break;
						case PINCEL_MOVER_CAMERA:/*JYH_Move_Camera(jogo,jogo->evt.motion.xrel,jogo->evt.motion.yrel)*/JYH_Move_Camera(&jogo->le.cam,&jogo->le.lvl,jogo->evt.motion.xrel,jogo->evt.motion.yrel);break;
					}
				}
				break;
